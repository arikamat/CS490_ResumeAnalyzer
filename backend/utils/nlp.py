...
def calculate_match_score(job_keywords, resume_keywords, weights):
    ...
    Args:
        job_keywords (dict): A dictionary with list of job keywords categorized by skills, 
                             experience, and education.
        resume_keywords (dict): A dictionary with list of job keywords categorized by skills, 
                             experience, and education.
    ...
    """
    processed_resume_keywords = {category: process_keywords(keywords) for category, keywords in resume_keywords.items()}
...
def calculate_fit_score(user_input: UserInput):
    ...
    Returns:
        fit_score (float): The overall fit score, a weighted value indicating the 
              alignment between the resume and job description.
        missing_schema (CategoricalKeyword): An object containing the missing 
              keywords for each category (skills, experience, education).

    Workflow:
        1. Ask AI to extract relevant keywords from job description and resume and categorize it
        2. Splits multi-word keywords into individual words, removes duplicates, and 
           convert them to lowercase for consistent comparison.
    ...
    for category in CATEGORIES:
        job_keywords[category] = [word for string in job_keywords[category] for word in string.split()]
        resume_keywords[category] = [word for string in resume_keywords[category] for word in string.split()]
    ...
    fit_score, detailed_scores, missing = calculate_match_score(job_keywords, resume_keywords, WEIGHTS)
