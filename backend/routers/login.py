...
async def check_login(login: Login):
    ...
    if login.email not in database:
        raise HTTPException(
            status_code=400, detail="No account associated with this email"
        )

    # if the password given is not the correct password
    if not bcrypt.checkpw(
        login.password.encode("utf-8"), database[login.email]["password"]
    ):
        raise HTTPException(status_code=400, detail="Incorrect password")

    # create payload for jwt token that will expire in 15 minutes
    payload = {"email": login.email, "exp": datetime.utcnow() + timedelta(hours=0.25)}
