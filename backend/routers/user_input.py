...
async def accept_user_input(user_input: UserInput):
    ...
    if not user_input.job_description:
        raise HTTPException(status_code=400, detail={"error": "Missing job description"})

    #if resume input is too long
    if len(user_input.resume_text)>10000:
        raise HTTPException(status_code=400, detail={"error": "Resume text is over the 10,000 character limit"})
    
    #if job description is too long
    if len(user_input.job_description)>10000:
        raise HTTPException(status_code=400, detail={"error": "Job description is over the 10,000 character limit"})
    ...
    if "error" in response_json:
        raise HTTPException(status_code=500, detail={"error": "Unable to process request at this time. Please try again later."})
