...
test("Signup, Login, and Upload Flow", async ({ page }) => {
  ...
  const testJobDescription =
    "This is a test job description for validation purposes.";
  ...
  const textArea = page.locator(
    'textarea[placeholder="Enter job description here..."]',
  );
  await textArea.fill(testJobDescription);
  const charInfo = page.locator(".char-info p:first-child");
  await expect(charInfo).toHaveText(
    `${testJobDescription.length} / 5000 characters`,
  );
