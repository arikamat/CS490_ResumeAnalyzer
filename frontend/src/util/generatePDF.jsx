...
export function generatePDF(fitScore, matchedKeywords, missingKeywords, improvementSuggestions) {
    const doc = new jsPDF();
    ...
    Object.keys(matchedKeywords).forEach((category) => {
        doc.text(`${category.charAt(0).toUpperCase() + category.slice(1)}:`, 15, currentY);
        currentY += 6;
    ...
    Object.keys(improvementSuggestions).forEach((category) => {
        ...
        Object.keys(improvementSuggestions[category]).forEach((keyword) => {
            const wrappedText = doc.splitTextToSize(`- ${keyword}: ${improvementSuggestions[category][keyword]}`, 180);
            doc.text(wrappedText, 20, currentY);
            currentY += wrappedText.length * 6;
        });
    });
