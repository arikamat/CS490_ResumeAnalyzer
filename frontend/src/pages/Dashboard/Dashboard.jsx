    ...
    <div className="dashboardContainer">
      ...
      <div className="panelsContainer">
        <div className="panel">
          <h3>Fit Score</h3>
          <p>
            Your resume matches {fitScore.toFixed(2)}% of the job description.
          </p>
        ...
        <div className="panel">
          ...
          {Object.keys(renderFilteredKeywords(missingKeywords)).length > 0 ? (
            <ul>
              {Object.keys(renderFilteredKeywords(missingKeywords)).map(
                (category) => (
                  <li key={category}>
                    <strong>
        ...
        <div className="panel">
          ...
          {Object.keys(renderFilteredKeywords(matchedKeywords)).length > 0 ? (
            <ul>
              {Object.keys(renderFilteredKeywords(matchedKeywords)).map(
                (category) => (
                  <li key={category}>
                    <strong>
                      {category.charAt(0).toUpperCase() + category.slice(1)}:{" "}
        ...
        <div className="panel">
          ...
          {Object.keys(renderFilteredKeywords(improvementSuggestions)).length >
          0 ? (
            Object.keys(renderFilteredKeywords(improvementSuggestions)).map(
              (category) => (
                <div key={category}>
                  <h4>
                    {category.charAt(0).toUpperCase() + category.slice(1)}
                  ...
                  <ul>
                    {Object.keys(improvementSuggestions[category]).map(
                      (keyword, index) => (
                        <li key={index} title={getTooltipText(category)}>
