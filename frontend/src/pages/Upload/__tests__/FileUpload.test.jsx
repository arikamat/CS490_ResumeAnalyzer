...
describe("FileUpload Component", () => {
  ...
  it("display an error for not pdf file upload", async () => {
    ...
    const invalidFile = new File(["content"], "safnoor.txt", {
      type: "text/plain",
    });
  ...
  it("display error for file over 2MB", async () => {
    ...
    expect(
      await screen.findByText(/File size must be under 2 MB/),
    ).toBeInTheDocument();
  });
  ...
  it("enable upload button for valid PDF file", async () => {
    ...
    const fileInput = screen.getByLabelText("Upload File");
    const validFile = new File(["content"], "safnoor.pdf", {
      type: "application/pdf",
    });
  ...
  it("display an error message on API fail", async () => {
    ...
    expect(
      await screen.findByText(/An error occurred during file upload./),
    ).toBeInTheDocument();
  });
  ...
  it("handle non 200 status code", async () => {
    ...
    expect(
      await screen.findByText(/Failed to upload the file./),
    ).toBeInTheDocument();
  });
